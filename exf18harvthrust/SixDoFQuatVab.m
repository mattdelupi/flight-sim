function [time, ...
    ...
    ThrustModel = @(t, state) delta_T_law(t) * myAircraft.ThrustModel(altitude(state), mach(state));
    ...
    Vdot = @(t, state) 1/m * (-DragModel(t, state)*cos(state(3)) + ...
        CrossforceModel(t, state)*sin(state(3)) + ThrustModel(t, state)*cos(state(2))*cos(state(3)) - ...
        W * (cos(state(2))*cos(state(3))*sin(theta(state)) - ...
             sin(state(3))*sin(phi(state))* ...
                                        cos(theta(state)) - ...
             sin(state(2))*cos(state(3))*cos(phi(state))* ...
                                        cos(theta(state))));

    alphadot = @(t,state) 1/(m*state(1)*cos(state(3))) * (-LiftModel(t, state) - ...
            ThrustModel(t, state)*sin(state(2)) + W * (cos(state(2))* ...
            cos(phi(state))*cos(theta(state)) + ...
            sin(state(2))*sin(theta(state)))) + ...
        state(5) - tan(state(3)) * ...
                        (state(4)*cos(state(2)) + state(6)*sin(state(2)));

    betadot = @(t, state) 1/(m*state(1)) * (DragModel(t, state)*sin(state(3)) + ...
        CrossforceModel(t, state)*cos(state(3)) - ThrustModel(t, state)*cos(state(2))*sin(state(3)) + ...
        W * (cos(state(2))*sin(state(3))*sin(theta(state)) + ...
             cos(state(3))*sin(phi(state))* ...
                                        cos(theta(state)) - ...
             sin(state(2))*sin(state(3))*cos(phi(state))* ...
                                        cos(theta(state)))) + ...
