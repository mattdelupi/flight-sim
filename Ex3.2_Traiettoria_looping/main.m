...
for it = 1 : length(v_time)
    V_E = T_EB(m_quat(it, :)) * ...
                            [u(v_time(it)); v(v_time(it)); w(v_time(it))];
    v_u_E(it) = V_E(1);
    v_v_E(it) = V_E(2);
    v_w_E(it) = V_E(3);
end
...
plotTrajectoryAndBodyE(h_fig1, shape, ...
...
plotEarthAxes(h_fig1, v_XYZ_0, v_extent);
hold off

% Figure add-ons
ylim([-80, 80])
xlabel("$x_{_E}$ (m)", 'Interpreter', 'latex'); ylabel("$y_{_E}$ (m)", 'Interpreter', 'latex'); zlabel("$z_{_E}$ (m)", 'Interpreter', 'latex')
%title("\textbf{3D View of the trajectory}", 'Interpreter', 'latex')
set(gca, 'TickLabelInterpreter', 'latex')
exportgraphics(gca, 'ex32main.pdf', 'Resolution', 450)