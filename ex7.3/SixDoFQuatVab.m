function [time, ...
    ...
    alpha_deg = @(state) convang(state(2), 'rad/s', 'deg/s');
    beta_deg = @(state) convang(state(3), 'rad/s', 'deg/s');
    ...
    Vdot = @(t, state) 1/m * (-D(t, state)*cos(state(3)) + ...
        C(t, state)*sin(state(3)) + T(t)*cos(state(2))*cos(state(3)) - ...
        W * (cos(state(2))*cos(state(3))*sin(quat2theta(state(10:13).')) - ...
             sin(state(3))*sin(quat2phi(state(10:13).'))* ...
                                        cos(quat2theta(state(10:13).')) - ...
             sin(state(2))*cos(state(3))*cos(quat2phi(state(10:13).'))* ...
                                        cos(quat2theta(state(10:13).'))));

    alphadot = @(t,state) 1/(m*state(1)*cos(state(3))) * (-L(t, state) - ...
            T(t)*sin(state(2)) + W * (cos(state(2))* ...
            cos(quat2phi(state(10:13).'))*cos(quat2theta(state(10:13).')) + ...
            sin(state(2))*sin(quat2theta(state(10:13).')))) + ...
        state(5) - tan(state(3)) * ...
                        (state(4)*cos(state(2)) + state(6)*sin(state(2)));

    betadot = @(t, state) 1/(m*state(1)) * (D(t, state)*sin(state(3)) + ...
        C(t, state)*cos(state(3)) - T(t)*cos(state(2))*sin(state(3)) + ...
        W * (cos(state(2))*sin(state(3))*sin(quat2theta(state(10:13).')) + ...
             cos(state(3))*sin(quat2phi(state(10:13).'))* ...
                                        cos(quat2theta(state(10:13).')) - ...
             sin(state(2))*sin(state(3))*cos(quat2phi(state(10:13).'))* ...
                                        cos(quat2theta(state(10:13).')))) + ...
