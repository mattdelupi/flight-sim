...
t_end = 90.5; % final time (s)
p_max = convangvel(5, 'deg/s', 'rad/s'); % max roll angular rate (rad/s)
q_max = convangvel(1.75, 'deg/s', 'rad/s'); % max pitch angular rate (rad/s)
u_0 = convvel(380, 'km/h', 'm/s');

%% p, q and r laws
p = @(t) interp1([0, 0.05, 0.2, 0.8, 0.95, 1] * t_end, ...
                 [0,  0,   1,   1,   0,  0] * p_max, ...
                  t, 'pchip');
...
legend("$p$", "$q$", 'Location', 'northeastoutside', 'Interpreter', 'latex')
axis tight
set(gca, 'TickLabelInterpreter', 'latex')
exportgraphics(gca, 'ex33main1.pdf', 'Resolution', 300)
...
axis tight
exportgraphics(gca, 'ex33main2.pdf', 'Resolution', 300)
...
% Settings
plot3d_options.samples = 31 : 30 : length(v_time);
...
plotTrajectoryAndBodyE(gcf, shape, ...
                                m_XYZ_EG, m_euler_angles, plot3d_options);
...
% Figure add-ons
% ylim([-80, 80])
xlabel("$x_{_\mathrm{E}}$ (m)", 'Interpreter', 'latex'); 
hyL = ylabel("$y_{_\mathrm{E}}$ (m)", 'Interpreter', 'latex'); hyL.Position = hyL.Position + [-450, -2250, 0.];
zlabel("$z_{_\mathrm{E}}$ (m)", 'Interpreter', 'latex')
%title("\textbf{3D View of the trajectory}", 'Interpreter', 'latex')
set(gca, 'TickLabelInterpreter', 'latex')
exportgraphics(gca, 'ex33main3.pdf', 'Resolution', 450)